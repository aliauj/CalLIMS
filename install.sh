...
create_env_file() {
  ...
  # CALLIMS_SECURE drives Django's TLS-dependent settings (SECURE_SSL_REDIRECT,
  # cookie SECURE flags, HSTS, CSRF_TRUSTED_ORIGINS scheme). Track --enable-https
  # so a re-run without the flag does not leave stale "secure" settings behind.
  if [[ "$ENABLE_HTTPS" == true ]]; then
    CALLIMS_SECURE_VALUE="True"
  else
    CALLIMS_SECURE_VALUE="False"
  fi
...
CALLIMS_SECURE=${CALLIMS_SECURE_VALUE}
